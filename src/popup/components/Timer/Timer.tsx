...
export function Timer({ timer, settings, onStart, onPause, onResume, onReset, onSkip }: Props) {
    ...
    <div className={styles.container}>
      ...
      <div className={styles.timerRing}>
        ...
        >
          ...
          <circle
            ...
            style={{
              ...
              transition: isRunning ? 'stroke-dashoffset 0.5s linear' : 'stroke-dashoffset 0.3s ease',
        ...
        <div className={styles.timerCenter}>
          <span className={styles.timeDisplay}>{formatTime(displayMs)}</span>
          {isRunning && (
      ...
      <div className={styles.cycleRow}>
        <div className={styles.cycleDots}>
          {Array.from({ length: settings.cycles }, (_, i) => {
            ...
            const active = i === timer.completedSessions && timer.phase === 'work' && !isIdle;
            return (
              <div
                key={i}
                className={`${styles.dot} ${done ? styles.dotDone : ''} ${active ? styles.dotActive : ''}`}
                style={done || active ? { background: color, boxShadow: done ? 'none' : `0 0 6px ${color}` } : undefined}
              />
            );
          })}
        </div>
      ...
      <div className={styles.controls}>
        ...
        {!isIdle && (
          <div className={styles.secondaryBtns}>
            <button className={styles.secondaryBtn} onClick={onSkip} title="Skip to next phase">
              <SkipIcon />
              Skip
            </button>
            <button className={styles.secondaryBtn} onClick={onReset} title="Reset timer">
              <ResetIcon />
              Reset
            </button>
          </div>
