...
import { AppState, AppMessage, DEFAULT_STATE, Settings } from '../shared/types';
...
export default function App() {
  ...
  useEffect(() => {
    chrome.runtime.sendMessage({ type: 'GET_STATE' } as AppMessage, (response) => {
  ...
  const isWorking = state.timer.status === 'running' && state.timer.phase === 'work';
    ...
    <div className={`${styles.root} ${theme === 'dark' ? styles.dark : styles.light}`}>
      ...
      <header className={styles.header}>
        <div className={styles.logo}>
          ...
          {isWorking && <span className={styles.activePill}>FOCUS</span>}
        </div>
            ...
            {tab === 'sites' && (
              <>
                ...
                {state.blockedSites.length > 0 && (
                  <span className={styles.badge}>{state.blockedSites.length}</span>
                )}
              </>
            )}
          ...
          <>
            ...
            {activeTab === 'sites' && (
              <SiteList
                ...
                onChange={(sites) => send({ type: 'UPDATE_SITES', sites })}
              />
            )}
            {activeTab === 'settings' && (
              <SettingsPanel
                ...
                onChange={(s: Partial<Settings>) => send({ type: 'UPDATE_SETTINGS', settings: s })}
              />
            )}
          </>
        )}
