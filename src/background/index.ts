...
const ALARM_NAME = 'pomoTimer';

// ─── Storage ─────────────────────────────────────────────────────────────────

async function getState(): Promise<AppState> {
  const data = await chrome.storage.local.get('appState');
  if (!data.appState) return { ...DEFAULT_STATE };
  return {
    settings: { ...DEFAULT_STATE.settings, ...data.appState.settings },
    blockedSites: data.appState.blockedSites ?? [],
    timer: { ...DEFAULT_STATE.timer, ...data.appState.timer },
  };
}
...
async function applyBlockingRules(sites: string[], active: boolean): Promise<void> {
...
async function handlePhaseEnd(): Promise<void> {
  ...
  if (timer.phase === 'work') {
    ...
    if (completed >= settings.cycles) {
      await startPhase('longBreak', settings.longBreak * 60_000, { completedSessions: completed });
    } else {
      await startPhase('shortBreak', settings.shortBreak * 60_000, { completedSessions: completed });
    }
  } else if (timer.phase === 'shortBreak') {
    await startPhase('work', settings.workTime * 60_000);
  } else {
...
chrome.runtime.onMessage.addListener(
  (message: AppMessage, _sender, sendResponse: (r: MessageResponse) => void) => {
...
async function handleMessage(msg: AppMessage): Promise<MessageResponse> {
  ...
  switch (msg.type) {
    ...
    case 'START': {
      ...
      const next = await startPhase('work', state.settings.workTime * 60_000, {
        completedSessions: 0,
      });
    ...
    case 'PAUSE': {
      ...
      await chrome.alarms.clear(ALARM_NAME);
      const next = await patchTimer({ status: 'paused', remainingMs, endTime: null });
      await applyBlockingRules(next.blockedSites, false);
      return { success: true, state: next };
    }

    case 'RESUME': {
      if (state.timer.status !== 'paused' || state.timer.remainingMs == null)
        return { success: true, state };
      const next = await startPhase(state.timer.phase, state.timer.remainingMs, {
    ...
    case 'UPDATE_SITES': {
      ...
      await saveState(next);
      const blocking = next.timer.status === 'running' && next.timer.phase === 'work';
